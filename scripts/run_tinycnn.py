#!/usr/bin/env python3
"""Train the from-scratch ImageCNN and export Grad-CAM / Grad-CAM++ figures.

Run:  python scripts/run_tinycnn.py
Outputs figures to results/tinycnn/<DATASET>/ and results/tinycnn/metrics.csv.
"""
...
INPUT_SIZE = 96
NUM_IMAGES = 10
OUTPUT_FOLDER = os.path.join("results", "tinycnn")
CHECKPOINT_FOLDER = "checkpoints"

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def train_model(model, train_loader, dataset_name, epochs):
...
def explain_one_image(model, test_dataset, class_names, dataset_name, image_index, method):
    ...
    heatmap, predicted_class = compute_gradcam_heatmap(
        model, image_tensor, model.conv3, INPUT_SIZE, target_class=None, method=method, device=DEVICE
    )
...
def main():
    ...
    cifar_model = train_model(ImageCNN(10), DataLoader(cifar_train, batch_size=128, shuffle=True), "CIFAR-10", epochs=5)
    test_model(cifar_model, DataLoader(cifar_test, batch_size=128), "CIFAR-10")
    torch.save(cifar_model.state_dict(), os.path.join(CHECKPOINT_FOLDER, "cifar10_model.pt"))

    stl_model = train_model(ImageCNN(10), DataLoader(stl_train, batch_size=128, shuffle=True), "STL-10", epochs=10)
    test_model(stl_model, DataLoader(stl_test, batch_size=128), "STL-10")
    torch.save(stl_model.state_dict(), os.path.join(CHECKPOINT_FOLDER, "stl10_model.pt"))
    ...
    print("\nGrad-CAM averages by dataset and method:")
